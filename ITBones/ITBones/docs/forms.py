...
class doc_f_docmaster(forms.ModelForm):
    doc_public = forms.CharField(widget=TinyMCE(attrs={'cols': 180, 'rows': 280}))
    ...
    doc_ref = forms.CharField(widget=TinyMCE(attrs={'cols': 180, 'rows': 20}),required = False)
    doc_sub = forms.CharField(widget=forms.TextInput(attrs={'size': '100', 'maxlength':'100'}))
    class Meta:
       ...
       # exclude = ('currprice','finalval','currvalue','stock_desc',)
        ...
        #}
class doc_f_files(forms.ModelForm):
    class Meta:
        model = doc_m_files
        fields = '__all__'
